    ...
    const startPolling = () => {
      ...
      pollingInterval = setInterval(async () => {
        try {
          ...
          // Also poll capacity sections for real-time updates
          console.log('📡 Polling capacity sections for real-time updates...');
          await fetchCapacitySections(bookingData.parkingArea.id);
        } catch (error) {
      ...
      try {
        ...
        if (targetReservationId) {
          ...
          if (response.success && response.data) {
            ...
            // Show grace period warning for newly created reservations
            console.log('🎯 Grace Period Debug - bookingData:', response.data);
            console.log('🎯 Grace Period Debug - bookingStatus:', response.data.bookingStatus);
            console.log('🎯 Grace Period Debug - timestamps:', response.data.timestamps);
            
            if (response.data.bookingStatus === 'reserved') {
              // Based on your database, the API likely returns timestamps.bookingTime
              const createdAt = response.data.timestamps?.bookingTime ||  // API field name
                               response.data.bookingTime ||              // Direct field
                               response.data.time_stamp ||               // Database field
                               response.data.timestamps?.time_stamp ||  // Nested database field
                               response.data.timestamps?.createdAt || 
                               response.data.createdAt || 
                               response.data.created_at;
              
              console.log('🎯 Grace Period Debug - createdAt:', createdAt);
              
              if (createdAt) {
                // Check if this is a recently created reservation (within last 2 minutes)
                const createdTime = new Date(createdAt).getTime();
                const now = Date.now();
                const timeSinceCreation = (now - createdTime) / (1000 * 60); // minutes
                
                console.log('🎯 Grace Period Debug - timeSinceCreation:', timeSinceCreation);
                
                if (timeSinceCreation <= 2) {
                  console.log('🎯 ActiveParkingScreen: New reservation detected, showing grace period warning');
                  showGracePeriodWarningModal(createdAt);
                } else {
                  console.log('🎯 Grace Period Debug - Reservation too old for warning:', timeSinceCreation, 'minutes');
                }
              } else {
                console.log('🎯 Grace Period Debug - No createdAt field found');
                // Show all available fields for debugging
                console.log('🎯 Grace Period Debug - Available fields:', Object.keys(response.data));
                if (response.data.timestamps) {
                  console.log('🎯 Grace Period Debug - Timestamp fields:', Object.keys(response.data.timestamps));
                }
              }
            }
          ...
          <View style={activeParkingScreenStyles.modalOverlay}>
            <View style={activeParkingScreenStyles.gracePeriodModalContainer}>
              ...
              <View style={activeParkingScreenStyles.gracePeriodModalContent}>
                ...
                <Text style={activeParkingScreenStyles.gracePeriodModalText}>
                  If you don't check in by this time, your reservation will be 
