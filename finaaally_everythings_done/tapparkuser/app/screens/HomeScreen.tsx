...
export default function HomeScreen() {
  ...
  // Real-time polling for frequent spots availability
  useEffect(() => {
    if (!isAuthenticated || frequentSpots.length === 0) {
      return;
    }

    const pollFrequentSpots = async () => {
      try {
        const response = await ApiService.getFrequentSpots(5);
        if (response.success && response.data?.frequent_spots) {
          setFrequentSpots(response.data.frequent_spots);
    ...
    const pollingInterval = setInterval(pollFrequentSpots, 10000);

    return () => clearInterval(pollingInterval);
  }, [isAuthenticated, frequentSpots.length > 0]);
  ...
  useEffect(() => {
    ...
    // Start polling every 10 seconds to update spot statuses and capacity sections
    const startPolling = () => {
      console.log('🔄 Starting real-time polling for HomeScreen parking data...');
      
      pollingInterval = setInterval(async () => {
        try {
          if (selectedAreaForSpots) {
            console.log('📡 Polling spot statuses for real-time updates...');
            await loadSpotStatuses(selectedAreaForSpots.id);
            
            // Also poll capacity sections for real-time updates
            console.log('📡 Polling capacity sections for real-time updates...');
            await fetchCapacitySections(selectedAreaForSpots.id);
          }
        } catch (error) {
